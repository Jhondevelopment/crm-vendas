    ...
    lista.innerHTML = avisosData.map(a => `
        <div class="aviso-item" style="flex-direction: column; align-items: flex-start; gap: 6px;">
            <div style="display: flex; justify-content: space-between; width: 100%;">
                ...
                ${(perfilAtual.role === 'gestor_geral') ? `<button class="aviso-del" title="Apagar aviso" onclick="window.deletarAviso(${a.id})"><i class="ph ph-trash"></i></button>` : ''}
            </div>
...
function renderizarBonus() {
    ...
    tbody.innerHTML = bonusData.map(b => {
        ...
        let botaoHTML = '';
        if (b.status === 'Ativa') {
            botaoHTML = `<button class="btn-cancel" style="padding:4px 8px; width:auto; margin:0 auto; font-size:12px; color:var(--danger); border-color:var(--danger);" onclick="window.encerrarBonus(${b.id})"><i class="ph ph-stop-circle"></i> Encerrar</button>`;
        } else {
            botaoHTML = `<span style="font-size:12px; color:var(--muted);">Encerrada</span>`;
        }
...
if (formBonus) {
    formBonus.onsubmit = async (e) => {
        ...
        if (error) {
            ...
            formBonus.reset(); 
            mostrarToast('Campanha de Bônus lançada!', 'ok'); 
            carregarBonus(); 
        }
    };
}
...
window.encerrarBonus = (id) => {
    window.abrirConfirmacao('Encerrar Campanha', 'Deseja parar esta campanha de bônus? Ela não aparecerá mais para os vendedores.', 'Encerrar', async () => {
...
function gerarCardHTML(l, status, isPosVenda = false) {
    ...
    if (l.estornado || status === 'perdidos') {
    ...
    if (l.is_recorrente) {
        iconesExtras += `<i class="ph ph-arrows-clockwise" title="Assinatura" style="color:#b388ff; margin-left:4px;"></i>`;
    }
...
// ==========================================
// CADASTRO E MOTIVO DE PERDA PERSONALIZADO ("Outro")
// ==========================================
...
if (formMotivoPerda) {
    formMotivoPerda.onsubmit = async (e) => {
        ...
        const btn = document.getElementById('form-motivo-perda').querySelector('button'); 
...
window.abrirDrawerLead = (id) => {
    ...
    if (lead.status === 'novos') {
        elStatus.classList.add('badge-azul'); 
    } else if (lead.status === 'negociacao') {
        elStatus.classList.add('badge-amarelo'); 
    } else if (lead.status === 'perdidos') {
        elStatus.classList.add('badge-vermelho'); 
    } else {
        elStatus.classList.add('badge-verde');
    }
    ...
    // Renderização de Arquivos (PIX/Cartão)
    const fileInputComprovante = document.getElementById('drawer-file-comprovante');
    if(fileInputComprovante) fileInputComprovante.value = '';
    
    document.getElementById('nome-arquivo-comprovante').innerText = 'Nenhum arquivo';
    ...
    if(lead.comprovante_url) { 
        ...
        document.getElementById('link-ver-comprovante').href = lead.comprovante_url; 
    } else { 
        boxVer.classList.add('hidden'); 
    }

    // Renderização de Contrato PDF
    const fileInputContrato = document.getElementById('drawer-file-contrato');
    if(fileInputContrato) fileInputContrato.value = '';
    
    document.getElementById('nome-arquivo-contrato').innerText = 'Sem contrato';
    document.getElementById('nome-arquivo-contrato').style.color = 'var(--muted)';
    
    const boxVerContrato = document.getElementById('box-ver-contrato');
    if(lead.contrato_url) { 
        boxVerContrato.classList.remove('hidden'); 
        document.getElementById('link-ver-contrato').href = lead.contrato_url; 
    } else { 
        boxVerContrato.classList.add('hidden'); 
    }
...
const fileInputComprovanteA = document.getElementById('drawer-file-comprovante');
if(fileInputComprovanteA) {
    fileInputComprovanteA.addEventListener('change', (e) => {
        ...
        if(e.target.files.length > 0) { 
            ...
            nameLabel.innerText = 'Nenhum arquivo'; 
...
const fileInputContrato = document.getElementById('drawer-file-contrato');
if(fileInputContrato) {
    fileInputContrato.addEventListener('change', (e) => {
        const nameLabel = document.getElementById('nome-arquivo-contrato');
        if(e.target.files.length > 0) { 
            nameLabel.innerText = e.target.files[0].name; 
            nameLabel.style.color = '#b388ff'; 
        } else { 
            nameLabel.innerText = 'Sem contrato'; 
            nameLabel.style.color = 'var(--muted)'; 
        }
    });
}
...
window.salvarDrawerLead = async () => {
    ...
    const fileInputComprovante = document.getElementById('drawer-file-comprovante');
    if (fileInputComprovante && fileInputComprovante.files.length > 0) {
        mostrarToast("Subindo comprovante financeiro...", "ok");
        const file = fileInputComprovante.files[0];
        const fileName = `lead_${id}_comp_${Date.now()}.${file.name.split('.').pop()}`; 
        
        const { data: uploadData, error: uploadError } = await supabase.storage.from('comprovantes').upload(fileName, file);
        if (uploadError) return mostrarToast("Falha no upload!", "erro");
        
        payload.comprovante_url = supabase.storage.from('comprovantes').getPublicUrl(fileName).data.publicUrl;
    }

    // Upload Contrato PDF
    const fileInputContratoVal = document.getElementById('drawer-file-contrato');
    if (fileInputContratoVal && fileInputContratoVal.files.length > 0) {
        mostrarToast("Guardando contrato no cofre...", "ok");
        const file = fileInputContratoVal.files[0];
        const fileName = `lead_${id}_contrato_${Date.now()}.${file.name.split('.').pop()}`; 
        
        const { data: uploadData, error: uploadError } = await supabase.storage.from('comprovantes').upload(fileName, file);
        if (uploadError) return mostrarToast("Falha no cofre!", "erro");
        
        payload.contrato_url = supabase.storage.from('comprovantes').getPublicUrl(fileName).data.publicUrl;
    }
...
window.gerarPropostaPDF = () => {
    ...
    if (!lead) {
        return mostrarToast('Lead não encontrado.', 'erro');
    }
...
function renderizarArena() {
    ...
    if (ranking[0].receita === 0) { 
        ...
        <div style="text-align: center; color: var(--muted); width:100%;">
            ...
            O pódio está vazio. Ninguém fechou vendas ainda!
        </div>`; 
        containerLista.innerHTML = ''; 
        return; 
    }
        ...
        containerLista.innerHTML = resto.map((v, i) => {
            ...
            <div class="seller-card">
                ...
                <div class="seller-stats">
                    ...
                    <div style="text-align:right;"><span>Faturamento</span><strong>${new Intl.NumberFormat('pt-BR', { style:'currency', currency:'BRL' }).format(v.receita)}</strong></div>
                </div>
            </div>`;
        }).join('');
    } else { 
...
function renderizarAgenda() {
    ...
    let leadsAgendados = leadsData.filter(l => l.data_followup && l.status !== 'fechados' && l.user_id === usuarioAtual.id);
    ...
    if (atrasados.length > 0) {
        ...
        <div class="agenda-group">
            <div class="agenda-group-title atrasado"><i class="ph ph-warning-circle"></i> Atrasados (Esfriando)</div>
            <div class="agenda-list">
                ${atrasados.map(l => gerarItemAgendaHTML(l, 'atrasado')).join('')}
            </div>
        </div>`;
    }
    ...
    if (hoje.length > 0) {
        html += `
        <div class="agenda-group">
            <div class="agenda-group-title hoje"><i class="ph ph-calendar-star"></i> Para Hoje</div>
            <div class="agenda-list">
                ${hoje.map(l => gerarItemAgendaHTML(l, 'hoje')).join('')}
            </div>
        </div>`;
    }
    ...
    if (futuros.length > 0) {
        html += `
        <div class="agenda-group">
            <div class="agenda-group-title"><i class="ph ph-calendar-blank"></i> Próximos Retornos</div>
            <div class="agenda-list">
                ${futuros.map(l => gerarItemAgendaHTML(l, 'futuro')).join('')}
            </div>
        </div>`;
    }
...
function calcularComissoesNovoModeloVendedor() {
    ...
    leadsDoUsuario.forEach(lead => {
        ...
        if(lead.status === 'fechados' && lead.aprovado === true && !lead.estornado) { 
            ganhoReal += valorCalculado; 
        } else if (lead.status !== 'fechados' && lead.status !== 'perdidos' && !lead.estornado) { 
            ganhoProjetado += valorCalculado; 
        }
    });
...
function renderizarComissoesNovoModeloFinanceiro() {
        ...
        chartPagamentosInstance = new Chart(ctxPagamentos, { 
            ...
            data: { 
                ...
                datasets: [{ 
                    ...
                    backgroundColor: ['#21a366', '#f5c518', '#8890aa'], 
                    borderWidth: 0 
                }] 
            }, 
            options: { 
                ...
                cutout: '70%', 
                plugins: { legend: { position: 'right', labels: { color: '#8890aa' } } } 
            } 
        });
    }
    ...
    if (vendedores.length === 0) { 
        ...
        if (!temDevida) { 
            tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 30px; color:var(--fechados);"><i class="ph ph-check-circle" style="font-size:24px; display:block;"></i>Todas as comissões pagas!</td></tr>`; 
        } else { 
...
window.estornarVenda = async (leadId) => {
    window.abrirConfirmacao('Estornar Venda (Chargeback)', '<strong style="color:var(--danger);">Atenção:</strong> Isso vai remover o valor do Faturamento, retirar a comissão do vendedor e marcar o cliente como PERDIDO. Confirma o estorno financeiro?', 'Estornar', async () => {
        const lead = leadsData.find(l => l.id == leadId); 
        let historicoAtual = lead.historico || []; 
        historicoAtual.push({ 
            data: new Date().toISOString(), 
            msg: `⚠️ ESTORNO/CHARGEBACK registrado por ${perfilAtual.full_name}` 
        });
        
        await supabase.from('leads').update({ 
            status: 'perdidos', 
            motivo_perda: 'Chargeback / Estorno Financeiro', 
            aprovado: false, 
            estornado: true, 
            historico: historicoAtual 
        }).eq('id', leadId);
...
window.aprovarVenda = async (id) => {
    ...
    historicoAtual.push({ 
        data: new Date().toISOString(), 
        msg: `Venda APROVADA no Financeiro por ${perfilAtual.full_name}` 
    });
...
window.rejeitarVenda = async (id) => {
    ...
    historicoAtual.push({ 
        data: new Date().toISOString(), 
        msg: `Venda REJEITADA no Financeiro por ${perfilAtual.full_name}` 
    });
...
function renderizarDespesas() {
    ...
    tbody.innerHTML = despesasData.map(d => {
        ...
        let btnAcao = d.status === 'Pendente' 
            ? `<button class="btn-primary" style="padding: 4px 8px; font-size: 12px; margin: 0 auto; width: auto;" onclick="window.quitarDespesa(${d.id})"><i class="ph ph-check"></i> Pagar</button>`
            : `<span style="font-size:12px; color:var(--muted);"><i class="ph ph-check-all"></i> Quitado</span>`;
        
        return `
        <tr>
            <td style="font-weight: bold; color: var(--text);">${d.descricao}</td>
            <td><span class="badge" style="background:rgba(255,255,255,0.05); color:var(--muted);">Fixo</span></td>
...
window.quitarDespesa = async (id) => {
    window.abrirConfirmacao('Quitar Despesa', 'Confirmar o pagamento desta conta?', 'Confirmar Pagamento', async () => {
        ...
        if (error) {
            mostrarToast('Erro ao atualizar conta.', 'erro');
        } else {
            mostrarToast('Despesa quitada com sucesso!', 'ok');
            carregarDespesas(); 
        }
    });
};
...
if (formDespesa) {
    formDespesa.onsubmit = async (e) => {
        ...
        if (error) {
            ...
            formDespesa.reset(); 
            mostrarToast('Despesa registrada.', 'ok'); 
            carregarDespesas(); 
        }
    };
}

// ==================== CFO: COBRANÇAS E INADIMPLÊNCIA ====================
function renderizarCobrancas() {
    ...
    if (inadimplentes.length === 0) { 
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding:30px; color:var(--muted);">Nenhum cliente inadimplente.</td></tr>`; 
        return; 
    }

    tbody.innerHTML = inadimplentes.map(l => {
        let numWpp = l.whatsapp.replace(/\D/g, ''); 
        if (!numWpp.startsWith('55') && numWpp.length <= 11) {
            numWpp = '55' + numWpp;
        }
...
window.marcarInadimplente = async (leadId) => {
    window.abrirConfirmacao('Sinalizar Inadimplência', 'Tem certeza que este cliente atrasou o pagamento ou o boleto falhou? Ele será enviado para a Central de Cobranças do Financeiro.', 'Sinalizar Atraso', async () => {
        ...
        historicoAtual.push({ 
            data: new Date().toISOString(), 
            msg: `⚠️ Marcado como INADIMPLENTE por ${perfilAtual.full_name}` 
        });
...
window.quitarInadimplencia = async (leadId) => {
    window.abrirConfirmacao('Baixa de Pagamento', 'O cliente realizou o pagamento do atraso? Isso vai tirá-lo da lista de inadimplentes.', 'Confirmar Pagamento', async () => {
        ...
        historicoAtual.push({ 
            data: new Date().toISOString(), 
            msg: `✅ Dívida Quitada. Marcado como PAGO por ${perfilAtual.full_name}` 
        });
...
function renderizarMRR() {
    ...
    let mrrAtivo = 0, mrrChurn = 0, qtdAtivos = 0; 
    let linhasHtml = '';
    ...
    if (assinantes.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:30px; color:var(--muted);">Nenhum cliente com assinatura (MRR) na base.</td></tr>`; 
    } else {
        tbody.innerHTML = linhasHtml;
    }
}
...
window.cancelarAssinatura = async (leadId) => {
    window.abrirConfirmacao('Registrar Churn', 'Tem certeza que este cliente cancelou a assinatura mensal? O valor sairá do cálculo de MRR.', 'Confirmar Cancelamento', async () => {
        ...
        historicoAtual.push({ 
            data: new Date().toISOString(), 
            msg: `❌ Assinatura Cancelada (Churn) por ${perfilAtual.full_name}` 
        });
...
function renderizarContratos() {
    ...
    const contratos = leadsData.filter(l => l.contrato_url && l.contrato_url !== null && l.contrato_url !== '');

    if (contratos.length === 0) { 
        tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding:30px; color:var(--muted);">O cofre está vazio. Nenhum contrato PDF foi anexado na gaveta dos clientes.</td></tr>`; 
        return; 
    }
    ...
    tbody.innerHTML = contratos.map(l => {
        ...
        <tr>
            ...
            <td style="text-align: right;">
                <a href="${l.contrato_url}" target="_blank" class="btn-anexo" style="border-color:#b388ff; color:#b388ff;">
    ...
    vendasAprovadas.forEach(lead => {
        ...
        if (precoVendido < precoOriginal) {
            ...
            <tr>
                ...
                <td style="text-align:center; ${corDesconto}">${porcentagemDesconto.toFixed(1)}% OFF</td>
            </tr>`;
        }
    });

    if (!temDesconto) {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 40px; color:var(--fechados);"><i class="ph ph-shield-check" style="font-size:32px; display:block; margin-bottom:8px;"></i>Nenhum vendedor aplicou descontos na base original. Margem protegida!</td></tr>`; 
    } else {
        tbody.innerHTML = linhasHtml;
    }
}
...
function renderizarTabelaExportacao() {
    ...
    tbody.innerHTML = list.map(l => `
        <tr>
            ...
            <td><span class="badge ${l.status === 'novos' ? 'badge-azul' : (l.status === 'negociacao' ? 'badge-amarelo' : (l.status === 'perdidos' || l.estornado ? 'badge-vermelho' : 'badge-verde'))}">${l.estornado ? 'ESTORNADO' : l.status.toUpperCase()}</span></td>
            <td style="color: var(--muted); font-size:12px;">${l.created_at ? new Date(l.created_at).toLocaleDateString('pt-BR') : '-'}</td>
        </tr>`).join('');
}
...
async function registrarAuditoriaExportacao(tipo, qtd) { 
    await supabase.from('logs_exportacao').insert([{ usuario: perfilAtual.full_name, tipo: tipo, quantidade: qtd }]); 
    carregarAuditoriaExportacao(); 
}
...
async function carregarAuditoriaExportacao() {
    ...
    const { data } = await supabase.from('logs_exportacao').select('*').order('created_at', { ascending: false }).limit(10); 
    logsExportacaoData = data || [];
    
    if(logsExportacaoData.length === 0) { 
        tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 20px; color:var(--muted);">Nenhuma exportação realizada ainda.</td></tr>`; 
        return; 
    }
    ...
    tbody.innerHTML = logsExportacaoData.map(log => `
        <tr>
            ...
            <td><strong style="color:var(--novos)">${log.quantidade}</strong> leads</td>
...
window.exportarCSV = () => {
    ...
    link.setAttribute("download", `relatorio_hapsis_${new Date().getTime()}.csv`); 
    document.body.appendChild(link); 
    link.click(); 
    document.body.removeChild(link);
    
    registrarAuditoriaExportacao('EXCEL', list.length); 
    mostrarToast('Planilha baixada!', 'ok');
};
...
window.exportarPDF = () => {
    ...
    <html>
        <head>
            ...
            <style>body{font-family:Arial;padding:40px;color:#333;}h1{border-bottom:2px solid #333;}table{width:100%;border-collapse:collapse;margin-top:20px;}th,td{padding:12px;border-bottom:1px solid #ddd;text-align:left;}th{background:#f8f9fa;}@media print{body{padding:0;}}</style>
        </head>
        <body>
            <h1>Relatório HAPSIS</h1>
            <p>Gerado por: ${perfilAtual.full_name} | Total: ${list.length}</p>
            <table>
                <thead>
                    <tr><th>Cliente</th><th>Contato</th><th>Produto</th><th>Valor</th><th>Status</th></tr>
                </thead>
                <tbody>
                    ${list.map(l => `<tr><td>${l.nome}</td><td>${l.whatsapp}</td><td>${l.produto||'-'}</td><td>R$ ${Number(l.valor).toFixed(2)}</td><td>${l.estornado ? 'ESTORNADO' : l.status}</td></tr>`).join('')}
                </tbody>
            </table>
...
function renderizarGestor() {
    ...
    if (ctxFugasLinha && ctxFugasMotivos) {
        ...
        chartFugasMotivosInstance = new Chart(ctxFugasMotivos, { 
            type: 'doughnut', 
            data: { 
                labels: Object.keys(contagemMotivos).length > 0 ? Object.keys(contagemMotivos) : ['Sem perdas'], 
                datasets: [{ 
                    data: Object.values(contagemMotivos).length > 0 ? Object.values(contagemMotivos) : [1], 
                    backgroundColor: ['#ff5c5c', '#ff8a65', '#e57373', '#f06292', '#ba68c8'], 
                    borderWidth: 0 
                }] 
            }, 
            options: { 
                ...
                maintainAspectRatio: false, 
                cutout: '70%', 
                plugins: { legend: { position: 'right', labels: { color: '#8890aa', font: {size: 10} } } } 
            } 
        });
        ...
        perdidos.forEach(l => {
            ...
            if (l.historico && l.historico.length > 0) { 
                const logPerda = l.historico.find(h => h.msg.includes('PERDIDO') || h.msg.includes('ESTORNO')); 
    ...
    if (document.getElementById('tabela-produtos')) { 
        if (arrayProds.length === 0) {
            document.getElementById('tabela-produtos').innerHTML = `<tr><td colspan="2" style="text-align:center; padding: 20px; color:var(--muted);">Nenhum produto fechado ainda.</td></tr>`; 
        } else {
...
function renderizarAbaAuditoria() {
    ...
    elTabela.innerHTML = leadsData.map(l => `
        <tr>
            ...
            <td><span class="badge ${l.status === 'novos' ? 'badge-azul' : (l.status === 'negociacao' ? 'badge-amarelo' : (l.status === 'perdidos' || l.estornado ? 'badge-vermelho' : 'badge-verde'))}">${l.estornado ? 'ESTORNADO' : l.status.toUpperCase()}</span></td>
            <td style="color:var(--muted);">${perfisEquipe.find(p => p.id === l.user_id)?.full_name || 'Deletado'}</td>
        </tr>`).join('');
}
...
function renderizarAbaEquipe() {
    ...
    perfisEquipe.forEach(p => { 
        const ne = p.equipe || 'Geral'; 
        if (!equipesAgrupadas[ne]) {
            equipesAgrupadas[ne] = []; 
        }
        equipesAgrupadas[ne].push(p); 
    });
    ...
    container.innerHTML = Object.entries(equipesAgrupadas).map(([nomeEquipe, membros]) => {
        ...
        membros.forEach(p => { 
            total += fechados.filter(l => l.user_id === p.id).reduce((s, l) => s + Number(l.valor), 0); 
            ativos += leadsData.filter(l => l.user_id === p.id && l.status !== 'fechados' && l.status !== 'perdidos' && !l.estornado).length; 
        });
        
        const sId = nomeEquipe.replace(/[^a-zA-Z0-9]/g, '-');
        
        return `
        <div class="folder-card" id="folder-card-${sId}">
            <div class="folder-header" onclick="window.toggleFolder('${sId}')">
                <div class="folder-title"><i class="ph ph-caret-right"></i> 📁 ${nomeEquipe.toUpperCase()} <span style="font-size:11px; font-weight:normal; color:var(--muted); margin-left:8px;">${membros.length} Membros | ${ativos} Leads</span></div>
                <div style="display:flex; align-items:center; gap:16px;">
                    <div class="folder-total">Receita: ${new Intl.NumberFormat('pt-BR', { style:'currency', currency:'BRL' }).format(total)}</div>
                    ${(nomeEquipe !== 'Geral' && perfilAtual.role === 'gestor_geral') ? `<button onclick="event.stopPropagation(); window.excluirEquipe('${nomeEquipe}')" style="background:transparent; border:none; color:var(--danger); font-size:20px; cursor:pointer;"><i class="ph ph-trash"></i></button>` : ''}
                </div>
            </div>
            <div class="folder-content" id="folder-content-${sId}">
                <table>
                    <thead>
                        <tr><th>Colaborador</th><th>Perfil</th><th>Vendido</th><th style="text-align: right;">Gerenciar</th></tr>
                    </thead>
                    <tbody>
                        ${membros.map(perfil => {
                            let vend = fechados.filter(l => l.user_id === perfil.id).reduce((s, l) => s + Number(l.valor), 0);
                            let cg = perfil.role === 'gestor_geral' || perfil.role === 'gestor' ? 'color:var(--accent); font-weight:bold;' : (perfil.role === 'gestor_sub' || perfil.role === 'gerente' ? 'color:#b388ff; font-weight:bold;' : 'color:var(--muted);');
                            let roleStr = perfil.role === 'gestor_geral' || perfil.role === 'gestor' ? 'Gestor Geral' : (perfil.role === 'gestor_sub' || perfil.role === 'gerente' ? 'Gestor Sub' : 'Vendedor');
                            
                            return `
                            <tr>
                                <td style="font-weight: 500;">${perfil.full_name}</td>
                                <td style="${cg}">${roleStr.toUpperCase()}</td>
                                <td style="color:var(--fechados); font-weight:bold;">${new Intl.NumberFormat('pt-BR', { style:'currency', currency:'BRL' }).format(vend)}</td>
                                <td style="text-align: right; display:flex; justify-content:flex-end; gap:8px;">
                                    ${(perfilAtual.role === 'gestor_geral') ? `<button onclick="window.abrirModalEquipe('${perfil.id}', '${perfil.full_name}', '${nomeEquipe}', 0)" style="background: transparent; border: 1px solid var(--border); color: var(--text); padding: 4px 10px; border-radius: 6px; cursor: pointer; font-size: 12px;"><i class="ph ph-arrows-left-right"></i> Mover</button>${perfil.id !== usuarioAtual.id ? `<button onclick="window.excluirVendedor('${perfil.id}', '${perfil.full_name}')" style="background: rgba(255,92,92,0.1); border: 1px solid var(--danger); color: var(--danger); padding: 4px 10px; border-radius: 6px; cursor: pointer; font-size: 12px;"><i class="ph ph-trash"></i></button>` : ''}` : `<span style="font-size:11px; color:var(--muted); font-style:italic;">Acesso Restrito</span>`}
                                </td>
                            </tr>`;
                        }).join('')}
                    </tbody>
...
window.abrirModalNovaEquipe = () => { 
    document.getElementById('inp-add-squad-user').innerHTML = perfisEquipe.map(p => `<option value="${p.id}">${p.full_name} (${p.equipe || 'Geral'})</option>`).join(''); 
    document.getElementById('inp-add-squad-nome').value = ''; 
    document.getElementById('modal-nova-equipe').classList.add('ativa'); 
};

if (document.getElementById('form-nova-equipe')) { 
    document.getElementById('form-nova-equipe').onsubmit = async (e) => { 
        e.preventDefault(); 
        const { error } = await supabase.from('profiles').update({ equipe: document.getElementById('inp-add-squad-nome').value.trim() }).eq('id', document.getElementById('inp-add-squad-user').value); 
        
        if (error) {
            mostrarToast('Erro', 'erro'); 
        } else { 
            mostrarToast('Squad criado!', 'ok'); 
...
window.excluirEquipe = (n) => { 
    window.abrirConfirmacao('Desmanchar', `Deseja desmanchar a equipe <strong>${n}</strong>?`, 'Desmanchar', async () => { 
        await supabase.from('profiles').update({ equipe: 'Geral' }).eq('equipe', n); 
        mostrarToast('Desmanchada!', 'ok'); 
...
window.excluirVendedor = (id, nome) => { 
    window.abrirConfirmacao('Remover', `Remover <strong>${nome}</strong>?`, 'Remover', async () => { 
...
window.abrirModalEquipe = (id, nome, eq) => { 
    document.getElementById('nome-usuario-mover').innerText = nome; 
    document.getElementById('inp-equipe-userid').value = id; 
    document.getElementById('inp-equipe-select').innerHTML = [...new Set(perfisEquipe.map(p => p.equipe || 'Geral'))].map(e => `<option value="${e}" ${e === eq ? 'selected' : ''}>${e}</option>`).join(''); 
    document.getElementById('inp-equipe-nova').value = ''; 
    document.getElementById('modal-mudar-equipe').classList.add('ativa'); 
};

if (document.getElementById('btn-fechar-modal-equipe')) {
    document.getElementById('btn-fechar-modal-equipe').onclick = () => document.getElementById('modal-mudar-equipe').classList.remove('ativa');
}

if (document.getElementById('form-mudar-equipe')) { 
    document.getElementById('form-mudar-equipe').onsubmit = async (e) => { 
        e.preventDefault(); 
        const vNovo = document.getElementById('inp-equipe-nova').value.trim(); 
        const vF = vNovo !== '' ? vNovo : document.getElementById('inp-equipe-select').value; 
        
        await supabase.from('profiles').update({ equipe: vF }).eq('id', document.getElementById('inp-equipe-userid').value); 
...
window.abrirPainelAcessos = () => {
    ...
    tbody.innerHTML = perfisEquipe.map(p => `
        <tr>
            <td style="font-weight:bold;">${p.full_name}</td>
            <td><span class="badge badge-equipe">${p.equipe || 'Geral'}</span></td>
            <td><span class="badge" style="${p.role === 'gestor_geral' || p.role === 'gestor' ? 'color:var(--accent);' : (p.role === 'gestor_sub' || p.role === 'gerente' ? 'color:#b388ff;' : 'color:var(--muted);')}">${p.role === 'gestor_geral' || p.role === 'gestor' ? 'GESTOR GERAL' : (p.role === 'gestor_sub' || p.role === 'gerente' ? 'GESTOR SUB' : 'VENDEDOR')}</span></td>
            <td style="text-align: right;">${p.id === usuarioAtual.id ? `<span style="font-size: 12px; color: var(--muted); font-style: italic;">Você</span>` : `<select onchange="window.mudarCargoUsuario('${p.id}', '${p.full_name}', this.value)" style="background:#0d0f18; border:1px solid var(--border); color:var(--text); padding:4px 8px; border-radius:6px; font-size:12px;"><option value="vendedor" ${p.role==='vendedor'?'selected':''}>Vendedor</option><option value="gestor_sub" ${(p.role==='gestor_sub'||p.role==='gerente')?'selected':''}>Gestor Sub</option><option value="gestor_geral" ${(p.role==='gestor_geral'||p.role==='gestor')?'selected':''}>Gestor Geral</option></select>`}</td>
        </tr>`).join('');
    
    document.getElementById('modal-acessos').classList.add('ativa');
};

window.mudarCargoUsuario = async (id, n, c) => { 
    await supabase.from('profiles').update({ role: c }).eq('id', id); 
    mostrarToast(`Nível de ${n} atualizado!`, 'ok'); 
...
// ==========================================
// CONFIGURAÇÕES (WHITE LABEL E CORES)
// ==========================================
function renderizarConfiguracoes() {
    const perfilGestor = perfisEquipe.find(p => p.role === 'gestor_geral') || perfilAtual;
    document.getElementById('inp-config-logo').value = perfilGestor.logo_empresa || '';
...
if (document.getElementById('form-config')) {
    document.getElementById('form-config').onsubmit = async (e) => {
        e.preventDefault();
        const btn = document.getElementById('form-config').querySelector('button'); 
        btn.innerHTML = 'Aplicando...';
        
        const { error } = await supabase.from('profiles').update({ 
            logo_empresa: document.getElementById('inp-config-logo').value, 
            cor_primaria: document.getElementById('inp-config-cor').value 
        }).eq('id', usuarioAtual.id);
        
        btn.innerHTML = '<i class="ph ph-check-circle"></i> Aplicar Identidade Visual';
        
        if (error) {
            mostrarToast('Erro ao salvar configurações.', 'erro'); 
        } else { 
            mostrarToast('Marca atualizada!', 'ok'); 
...
// ==========================================
// BARRA MÁGICA (COMANDOS CTRL+K) E NOTIFICAÇÕES
// ==========================================
...
const comandosDisponiveis = [
    ...
    { id: 'ir-cobrancas', titulo: 'Central de Cobranças', sub: 'Gerenciar inadimplência', icone: 'ph-warning-octagon', roles: ['gestor_sub'], acao: () => document.querySelector('[data-aba="aba-cobrancas"]').click() },
...
function renderizarComandos() {
    if (comandosFiltrados.length === 0) { 
        cmdList.innerHTML = `<div style="padding: 30px; text-align: center; color: var(--muted);">Sem comandos encontrados.</div>`; 
        return; 
    }
    
    cmdList.innerHTML = comandosFiltrados.map((cmd, i) => `
        <div class="cmd-item ${i === indiceSelecionado ? 'selecionado' : ''}" onclick="window.executarCmdClick(${i})" onmouseenter="window.setCmdIndex(${i})">
...
function filtrarComandos(t) { 
    const b = t.toLowerCase(); 
    comandosFiltrados = comandosDisponiveis.filter(c => c.roles.includes(perfilAtual.role) && (c.titulo.toLowerCase().includes(b) || c.sub.toLowerCase().includes(b))); 
    indiceSelecionado = 0; 
    renderizarComandos(); 
}

window.executarCmdClick = (i) => executarComando(i); 

window.setCmdIndex = (i) => { 
    indiceSelecionado = i; 
...
if (document.getElementById('btn-abrir-comandos')) {
    document.getElementById('btn-abrir-comandos').onclick = () => abrirBarraMagica();
}
...
function configurarNotificacoes() {
        ...
        if (payload.new.status === 'fechados' && payload.old.status !== 'fechados') {
            ...
            if (perfilAtual && (perfilAtual.role === 'gestor_geral' || perfilAtual.role === 'gestor_sub')) {
                ...
                if (document.getElementById('notif-panel') && document.getElementById('notif-panel').classList.contains('hidden')) { 
                    notifsNaoLidas++; 
                    document.getElementById('notif-count').innerText = notifsNaoLidas; 
                    document.getElementById('notif-count').classList.add('tem-notif'); 
                }
            }
            carregarLeads(); 
        }
    ...
    supabase.channel('mudanca-cargo').on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, (payload) => {
        if (payload.new.id === usuarioAtual.id && payload.new.role !== perfilAtual.role) { 
            mostrarToast('Seu acesso mudou.', 'ok'); 
            setTimeout(()=> window.location.reload(), 2000); 
        }
        if (payload.new.id === usuarioAtual.id && payload.new.logo_empresa !== perfilAtual.logo_empresa) { 
            setTimeout(()=> window.location.reload(), 1000); 
        }
    }).subscribe();
}
...
// ==========================================
// FUNÇÕES DO WHATSAPP (ABRIR E ENVIAR COM TEXTO LIVRE)
// ==========================================
window.abrirModalWhatsApp = (numero, nome) => {
    ...
    // Limpa a caixa de texto livre para não acumular mensagem de outro cliente
    const customInput = document.getElementById('wpp-custom-msg');
...
window.enviarWhatsApp = (tipo) => {
    ...
    const nomeVendedor = perfilAtual ? perfilAtual.full_name : 'Nossa Equipe';
    const nomeEmpresa = perfilAtual ? perfilAtual.nome_empresa : 'HAPSIS';
    
    let mensagem = '';
    
    if (tipo === 'apresentacao') {
        mensagem = `Olá ${nome}, tudo bem? Sou o ${nomeVendedor} da ${nomeEmpresa}. Vi que você demonstrou interesse em nossos serviços. Podemos conversar?`;
    } else if (tipo === 'followup') {
        mensagem = `Oi ${nome}! Passando para saber se conseguiu dar uma olhada na nossa proposta. Ficou alguma dúvida?`;
    } else if (tipo === 'cobranca') {
        mensagem = `Olá ${nome}, tudo bem? Notamos uma pendência no seu último pagamento. Como podemos te ajudar a regularizar?`;
    } else if (tipo === 'custom') {
        // Pega o texto livre que o vendedor digitou
        mensagem = document.getElementById('wpp-custom-msg').value;
        if (!mensagem.trim()) {
            mostrarToast('Digite uma mensagem antes de enviar.', 'erro');
            return; // Impede que envie vazio
        }
    }
